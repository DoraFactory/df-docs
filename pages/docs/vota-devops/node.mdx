...
| :----------- | :------ | :--- | :--------- |
| vota-ash     | 8 cores | 32GB | 1TB NVME   |
| vota-testnet | 4 cores | 16GB | 500GB NVME |
| vota-sf      | 8 cores | 32GB | 500GB NVME |
...
<Tabs items={['Mainnet', 'Testnet', 'Incentivized Testnet']}>
  <Tabs.Tab>
    - If you use an **AWS cloud server** to deploy a validator node, you can use the following recommended configuration: [`t2.2xlarge`](https://instances.vantage.sh/aws/ec2/t2.2xlarge)
    - If you use a **GCP cloud server** to deploy a validator node, you can use the following recommended configuration: [`n2-standard-8`](https://cloud.google.com/compute/docs/general-purpose-machines#n2_series)
  </Tabs.Tab>
  <Tabs.Tab>
...
- Incentivized Testnet: Replace the genesis file in `~/.dora/config` with the [Dora Vota Testnet genesis file](https://df-node-dump.s3.ap-southeast-1.amazonaws.com/vota-sf-genesis.json).
...
<Tabs items={['Mainnet', 'Testnet', 'Incentivized Testnet']}>
  ...
  ```shell
  # Comma separated list of seed nodes to connect to
  seeds = "059e1ec595989c8c4ef3fd3552070f5f93888547@3.79.184.10:26656"
  
  # Comma separated list of nodes to keep persistent connections to
  persistent_peers = "059e1ec595989c8c4ef3fd3552070f5f93888547@3.79.184.10:26656"
  ```
  </Tabs.Tab>
</Tabs>

The configuration above is mainly used for P2P node discovery. Before starting the node, you can synchronize it to a specified block height using the [snapshot](https://df-docs.vercel.app/docs/vota-devops/snapshot) service. Then, you can synchronize to the latest block using the P2P nodes configured above, which reduces the synchronization time.
