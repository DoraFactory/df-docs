...
desc: 'In this guide, we will walk you through the process of setting up and running a Dora Vota full node.'
---
...
The following information describes the most important node configuration settings found in the `~/.dora/config/` directory. This is the structure if `~/.dora/config`:
