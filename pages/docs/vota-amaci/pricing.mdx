...
- **5% of the total service fee** will be refunded for **_each miss_**.
- **90% of the total service fee** (i.e., the entire operator service provider fee) will be refunded if the tally is not completed within 72 hours after the round ends.

A **_miss_** is defined as any of the following conditions:

- A request to update an aMACI key (specifically, a voter’s deactivate key request) is delayed by more than **3 minutes**.
- After the round ends, the tally process is delayed by more than **30 minutes**.
...
- Exceptional cases (manually claim):
  - For example, the operator service remains unavailable for **over 72 hours** after the round ends.
