...
$ make install
$ sudo cp <your_dorad_path> /usr/local/bin
```
...
```ansi filename="Terminal"
$ dorad tx bank send <from_addr> <to_addr> <amount> --chain-id vota-ash --gas-prices=10000000000peaka --gas=auto --gas-adjustment=1.5 --node https://vota-rpc.dorafactory.org:443
```
