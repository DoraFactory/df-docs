---
title: 'Public Resources · Dora Vota Developer Guide'
desc: 'This beginner-friendly guide is intended to help you get started with Dora Vota and build on it.'
---

import { Tabs } from 'nextra/components'

# Public Resources

There are several public resources available for development.

## Networks

- Dora Vota Mainnet, identified with the chain ID: `vota-ash`
- Dora Vota Testnet, identified with the chain ID: `vota-testnet`
- Dora Vota Incentivized Testnet, identified with the chain ID: `vota-bobtail`

## Genesis files

- Mainnet: [`vota-ash`](https://github.com/DoraFactory/doravota/blob/main/config/mainnet/genesis.json)
- Testnet: [`vota-testnet`](https://github.com/DoraFactory/doravota/blob/main/config/testnet/genesis.json)
- Incentivized Testnet: [`vota-bobtail`](https://df-node-dump.s3.ap-southeast-1.amazonaws.com/vota-bobtail-genesis.json)

## Endpoints

### RPC

{/* prettier-ignore */}
<Tabs items={['Mainnet', 'Testnet', 'Incentivized Testnet']}>
  <Tabs.Tab>
    ```text
    https://vota-rpc.dorafactory.org:443
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```text
    https://vota-testnet-rpc.dorafactory.org:443
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```text
    https://vota-bobtail-rpc.dorafactory.org:443
    ```
  </Tabs.Tab>
</Tabs>

### gRPC

{/* prettier-ignore */}
<Tabs items={['Mainnet', 'Testnet', 'Incentivized Testnet']}>
  <Tabs.Tab>
    ```text
    vota-grpc.dorafactory.org:443
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```text
    vota-testnet-grpc.dorafactory.org:443
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```text
    vota-bobtail-grpc.dorafactory.org:443
    ```
  </Tabs.Tab>
</Tabs>

### REST API

{/* prettier-ignore */}
<Tabs items={['Mainnet', 'Testnet', 'Incentivized Testnet']}>
  <Tabs.Tab>
    ```text
    https://vota-rest.dorafactory.org:443
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```text
    https://vota-testnet-rest.dorafactory.org:443
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```text
    https://vota-bobtail-rest.dorafactory.org:443
    ```
  </Tabs.Tab>
</Tabs>

## Blockchain explorers

- Mainnet: https://vota-explorer.dorafactory.org
- Testnet: https://vota-testnet-explorer.dorafactory.org
- Incentivized Testnet: https://vota-bobtail-explorer.dorafactory.org

## Faucets

- Mainnet: https://dorahacks.io/faucet
- Testnet: https://vota-testnet-faucet.dorafactory.org

## CosmWasm documentation [#cosmwasm-docs]

- [CosmWasm Book](https://book.cosmwasm.com/)
- [Sylvia Book](https://cosmwasm.github.io/sylvia-book/index.html)
- [CosmWasm Academy](https://academy.cosmwasm.com/)
- [awesome-cosmwasm](https://github.com/CosmWasm/awesome-cosmwasm)
- [cosmwasm-std documentation](https://docs.rs/cosmwasm-std/latest/cosmwasm_std/)
