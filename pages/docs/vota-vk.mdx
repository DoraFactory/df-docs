...
<Callout>
  If you have already been invited to our Telegram channel, you can immediately
  start to run your node and sync block data. Please submit your address you
  wish to receive testnet tokens. We will airdrop `vota-vk` tokens so that you
  can create your validator and join the incentivized testnet.
</Callout>

<Callout type="info">
  For instructions on launching the incentivized testnet, please refer to this
  [document](/docs/vota-devops/validator). It is important to pay special
  attention to several parameters of the incentivized testnet that are different
  from `vota-ash` network, such as chain ID, RPC, and so on. Please also note
  that we use the `dorad` binary file with version 0.2.0 in the incentivized
  testnet. Relevant network configurations can be found in [public
  resources](/docs/vota-dev/resources).
</Callout>
